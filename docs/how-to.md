...
`c5fbb041b65ceab442b1df16e84a88b9365d71d8` (main):

```bash
curl -fsSL https://codeload.github.com/aql-lang/aql/tar.gz/c5fbb041b65ceab442b1df16e84a88b9365d71d8 | tar -xz
cd aql-c5fbb041b65ceab442b1df16e84a88b9365d71d8/cmd/go
...
git -C aql checkout c5fbb041b65ceab442b1df16e84a88b9365d71d8
...
This module is verified against aql commit `c5fbb04`; `aql:matrix-util`
...
Matrix must `import "aql:matrix-util"` themselves — the library does not
...
In a test, assert the failure (or its exact code) with `aql:test`:
...
`aql:struct-util` yourself — `stats.aql` pulls in its own dependencies:
...
`import "aql:matrix-util"` yourself, because the `MatrixUtil` binding is
...
```bash
aql test/stats_unit_test.aql   # example-based unit tests — direct (aql:test)
aql test/stats_unit_spec.aql   # example-based unit tests — declarative spec format
aql test/stats_prop_test.aql   # property tests — direct Test.check-prop form
aql test/stats_prop_spec.aql   # property tests — declarative spec format
aql test/stats_smoke_test.aql  # end-to-end walk-through over every public word
```
...
suite through all three of aql's execution surfaces — the interpreter,
`aql check` (static type-check), and the byte compiler (`aql --compile`)
