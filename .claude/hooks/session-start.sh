...
if [ -x "$BORU" ] && [ -f "$CLAUDE_PROJECT_DIR/test/stats_smoke_test.aql" ]; then
  if ( cd "$CLAUDE_PROJECT_DIR" && "$BORU" test/stats_smoke_test.aql >/dev/null 2>&1 ); then
    log "Smoke check passed (boru test/stats_smoke_test.aql)."
  else
